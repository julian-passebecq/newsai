...
def parse_sitemap(url, is_brunner_format=False):
    ...
    try:
        ...
        namespace = '' if not is_brunner_format else {'ns': 'http://www.sitemaps.org/schemas/sitemap/0.9'}
        sitemap_xml = ET.fromstring(response.content)
        articles = []
        url_tag = 'url' if not is_brunner_format else 'ns:url'
        
        for url_elem in sitemap_xml.findall(url_tag, namespace):
            loc = url_elem.find('loc' if not is_brunner_format else 'ns:loc', namespace).text
            lastmod_elem = url_elem.find('lastmod' if not is_brunner_format else 'ns:lastmod', namespace)
...
def main():
    ...
    for website, url in standard_sitemap_urls.items():
        ...
        for article in articles:
            article['Website'] = website  # Add website name to each article
    ...
    if all_articles:
        ...
        website_selection = st.multiselect('Select Websites', options=list(standard_sitemap_urls.keys()) + list(brunner_sitemap_urls.keys()), default=list(standard_sitemap_urls.keys()) + list(brunner_sitemap_urls.keys()))
